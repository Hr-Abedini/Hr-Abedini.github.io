...
function getJalaliDateParts(dateString) {
  ...
  return {
    ...
    formatted4: `${weekday} ${jd} ${monthName} ${jy}`,
  };
}
...
{
   let footers = document.querySelectorAll('.post-date');
   footers.forEach(oldSpan => {
                     
          let dateObj = convertTextToStandardDate(oldSpan.title)
          const parts = getJalaliDateParts(dateObj);
          
          if (!parts) return;

          oldSpan.title = parts.formatted3;    
          oldSpan.textContent=parts.formatted4;    
